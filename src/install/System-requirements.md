## Java ##
Most of the JPF components, including the [jpf-core](../jpf-core/JPF-core.md), are pure Java applications. The oldest version supported is Java SE 8 (in `java-8` branch) and the latest Java version supported is Java SE 11 (in `master` branch).
...
If you want to install the [Eclipse plugin](./Eclipse-Plugin.md), you need an Eclipse version >= 3.5 (Galileo) **running under JavaSE-1.8**. Please see the [Installing the Eclipse JPF plugin](./Eclipse-Plugin.md) page for details.
